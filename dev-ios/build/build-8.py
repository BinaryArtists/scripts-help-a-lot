...
# scheme
PROJECT_SCHEME = "XXXX"
# target(默认情况下，它和Scheme是一样的)
PROJECT_TARGET = "XXXX"
# 相关证书
# IN HOUSE
CODE_SIGN_IDENTITY_INHOUSE = "xxxx"
PROVISIONING_PROFILE_INHOUSE = "xxxx"#49cac5ce-3c77-472b-9abb-7c463a8bea92
CONFIGURATION_INHOUSE = "InHouse"
BUNDLE_ID_INHOUSE = "com.xxxx.xxxx"
INHOUSE_TEAM_ID = "xxxxx"#team id
# APP STORE
CODE_SIGN_IDENTITY_APP_STORE = "xxxx"
PROVISIONING_PROFILE_APP_STORE = "xxxx"
CONFIGURATION_APP_STORE = "Release"
BUNDLE_ID_APP_STORE = "com.xxxx.xxxx"
APPSTORE_TEAM_ID = "xxxxx"
...
# 打包ipa
def build_ipa(project_path, output_path, plist_path):
...
def modify_project_for_inhouse():
    #修改xcode签名方式
    modify_sign_string = "sed -i '' 's/ProvisioningStyle = Automatic;/ProvisioningStyle = Manual;/g' ./%s/project.pbxproj" % (
    PROJECT_NAME)
    os.system(modify_sign_string)
    #修改TeamId
    modify_team_id = "sed -i '' 's/%s/%s/g' ./%s/project.pbxproj" % (APPSTORE_TEAM_ID, INHOUSE_TEAM_ID, PROJECT_NAME)
    os.system(modify_team_id)
    #修改identity
    modify_identity = "sed -i '' 's/%s/%s/g' ./%s/project.pbxproj" % ("iPhone Developer", "iPhone Distribution", PROJECT_NAME)
    os.system(modify_identity)

#恢复修改内容
def restore_project():
    # 修改xcode签名方式
    modify_sign_string = "sed -i '' 's/ProvisioningStyle = Manual;/ProvisioningStyle = Automatic;/g' ./%s/project.pbxproj" % (
        PROJECT_NAME)
    os.system(modify_sign_string)
    # 修改TeamId
    modify_team_id = "sed -i '' 's/%s/%s/g' ./%s/project.pbxproj" % (INHOUSE_TEAM_ID, APPSTORE_TEAM_ID, PROJECT_NAME)
    os.system(modify_team_id)
    # 修改identity
    modify_identity = "sed -i '' 's/%s/%s/g' ./%s/project.pbxproj" % (
    "iPhone Distribution", "iPhone Developer", PROJECT_NAME)
    os.system(modify_identity)

#修改bundleId
def modify_bundle_id():
    modify_bundle = "sed -i '' 's/%s/%s/g' ./%s/project.pbxproj" % (BUNDLE_ID_APP_STORE,BUNDLE_ID_INHOUSE,PROJECT_NAME)
    print(modify_bundle)
    os.system(modify_bundle)

#还原bundleId
def restore_bundle_id():
    modify_bundle = "sed -i '' 's/%s/%s/g' ./%s/project.pbxproj" % (BUNDLE_ID_INHOUSE,BUNDLE_ID_APP_STORE, PROJECT_NAME)
    os.system(modify_bundle)
...
def build_inhouse(project_path):
    ...
    modify_bundle_id()
    #修改project
    modify_project_for_inhouse()
    # 编译coocaPods项目文件并 执行编译目录
    build_inhouse_workspace(project_path)
    # 导出ipa相关配置文件
    plist_path = '%s/enterprise.plist' % (project_path)
    # 打包ipa 并放到桌面
    file_name = build_ipa(project_path, get_output_path(project_path), plist_path)
    print("%s" % (file_name))
    if len(file_name) > 0:
        # 上传蒲公英
...
def build_appstore(project_path):
    ...
    plist_path = '%s/appstore.plist' % (project_path)
    # 打包ipa 并放到桌面
    build_ipa(project_path, get_output_path(project_path), plist_path)
